...
TacsateflaloPath  <- "S:/Onderzoek/Projecten VMS/BASEFILES/"                             #path were tacsatEflalo stands if used to define activities
...
load(file=file.path(shapePath,"grd.RData",sep=""))
...
if (actMethod =="tacsatActivity"){
  for (year in years){
    load(file.path(TacsateflaloPath,paste("tacsatActivity",year,".RData",sep="")))  #named eflalo as an object
  ...
  tacsatp.yrs <- merge(tacsatp.yrs,act.yrs)
}
